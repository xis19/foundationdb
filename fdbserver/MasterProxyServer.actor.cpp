...
ACTOR Future<Void> reply(Context* self) {
	...
	if (SERVER_KNOBS->ASK_READ_VERSION_FROM_MASTER && self->commitVersion > pProxyCommitData->committedVersion.get()) {
		wait(pProxyCommitData->master.reportLiveCommittedVersion.getReply(ReportRawCommittedVersionRequest(self->commitVersion, self->lockedAfter, self->metadataVersionAfter), TaskPriority::ProxyMasterVersionReply));
	}
	...
	for (int t = 0; t < trs.size(); t++) {
		if (self->committed[t] == ConflictBatch::TransactionCommitted && (!self->locked || self->trs[t].isLockAware())) {
			...
			trs[t].reply.send(CommitID(self->commitVersion, t, self->metadataVersionAfter));
