		...
		if (committed[transactionNum] == ConflictBatch::TransactionCommitted && (!locked || context.trs[transactionNum].isLockAware())) {
			...
			for (; mutationNum < pMutations->size(); mutationNum++) {
				...
				if (isSingleKeyMutation((MutationRef::Type) m.type)) {
					...
					if (debugMutation("ProxyCommit", context.commitVersion, m))
						TraceEvent("ProxyCommitTo", context.pProxyCommitData->dbgid).detail("To", describe(tags)).detail("Mutation", m.toString()).detail("Version", context.commitVersion);
					...
					if (firstRange == ranges.end()) {
						// Fast path
						if (debugMutation("ProxyCommit", context.commitVersion, m))
							TraceEvent("ProxyCommitTo", context.pProxyCommitData->dbgid).detail("To", describe(ranges.begin().value().tags)).detail("Mutation", m.toString()).detail("Version", context.commitVersion);
						...
						if (debugMutation("ProxyCommit", context.commitVersion, m))
							TraceEvent("ProxyCommitTo", context.pProxyCommitData->dbgid).detail("To", describe(allSources)).detail("Mutation", m.toString()).detail("Version", context.commitVersion);
	...
	if(context.pProxyCommitData->committedVersion.get() < context.commitVersion - SERVER_KNOBS->MAX_READ_TRANSACTION_LIFE_VERSIONS) {
		...
		while (context.pProxyCommitData->committedVersion.get() < context.commitVersion - SERVER_KNOBS->MAX_READ_TRANSACTION_LIFE_VERSIONS) {
			...
			choose{
				...
				when(GetReadVersionReply v = wait(context.pProxyCommitData->getConsistentReadVersion.getReply(GetReadVersionRequest(0, TransactionPriority::IMMEDIATE, GetReadVersionRequest::FLAG_CAUSAL_READ_RISKY)))) {
