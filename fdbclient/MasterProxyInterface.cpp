...
bool shouldSplitCommitTransaction(
        const CommitTransactionRequest& commitTxnRequest,
    ...
    const int size = std::accumulate(
        commitTxnRequest.transaction.mutations.begin(),
        commitTxnRequest.transaction.mutations.end(),
        0,
        [](int total, const MutationRef& ref) {
            return total + ref.param2.size();
        }
    );
 ...
 * the same splitID
 */
static std::vector<CommitTransactionRequest> prepareSplitTransactions(
        const CommitTransactionRequest& commitTxnRequest,
        const int numProxies) {
...
struct MutationValueSizeIndex {
    ...
    bool operator<(const MutationValueSizeIndex& another) const {
        return this->valueSize < another.valueSize;
    }
};
...
static void distributeMutations(
    ...
    using MutationValueSizeIndexHeap =
        std::priority_queue<MutationValueSizeIndex>;
    using MutationTotalValueSizeIndexHeap =
        std::priority_queue<
            MutationTotalValueSizeIndex,
            std::vector<MutationTotalValueSizeIndex>,
    ...
    for (auto i = 0;i < NUM_MUTATIONS; ++i) {
        valueSizeIndex.push(MutationValueSizeIndex{
            mutations[i].param2.size(), i
        });
    }
    ...
    for (auto i = 0; i < NUM_PROXIES; ++i)
        txnTotalValueSizeHeap.push(MutationTotalValueSizeIndex{0, i});
 ...
 */
std::vector<CommitTransactionRequest> splitTransaction(
        const CommitTransactionRequest& commitTxnRequest,
        const int numProxies) {

    std::vector<CommitTransactionRequest> result(prepareSplitTransactions(
        commitTxnRequest, numProxies
    ));
